const ACCENT = {
  teal: { bg: "bg-accent-teal", text: "text-accent-teal", glow: "animate-card-glow-teal" },
  purple: { bg: "bg-accent-purple", text: "text-accent-purple", glow: "animate-card-glow-purple" },
  orange: { bg: "bg-accent-orange", text: "text-accent-orange", glow: "animate-card-glow-orange" },
};
...
export default function StatusBanner({ accent = "teal", title, subtitle, cancelLabel = "Cancel", onCancel }) {
  const { bg, text, glow } = ACCENT[accent];
  return (
    <div className={`flex w-full items-center gap-4 rounded-2xl px-4 py-3 shadow-2xl ${bg} ${glow}`}>
